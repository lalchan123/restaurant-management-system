...
//   } = useOtpCheck();
  ...
  const [signin_loading, setSignInLoading] = useState(false);
  const [signin_error, setSignInError] = useState(false);
  ...
  const handleSignInSubmit = async (e) => {
    ...
    // dynamic URL
    const apiUrl = `${BaseURL}/account/user-sign-in/`;
    ...
    if (response_data.status == 200) {
      ...
      const EmailMessage ={
        ...
        phone: "01306817790",
        email: email,
        message: `Your OTP: ${response_data?.otp_data}\nCurrent TimeStand: ${d.getTime()}`,
      }
      ...
      toast.error(`Sign-in failed: ${response_data?.response?.data?.message}`);
      setErrorMessage(response_data?.response?.data?.message)
      setSignInError(true)
    }
  ...
  const handleOtpSubmit = async(e) => {
    ...
    const payload = {
      ...
      "user_otp": otpValue.otp
    }

    setOtpLoading(true)
    // dynamic URL
    const apiUrl = `${BaseURL}/account/user-otp-check-sign-in/`;
    const response_data = await restAPIPost(apiUrl, payload)
    console.log("128 response_data", response_data)
    if (response_data.status == 200) {
      setOtpCheck(false);
      // console.log("109 otp_success", otp_success)
      localStorage.setItem("email", response_data?.data?.login_data?.email);
      localStorage.setItem("user_id", response_data?.data?.login_data?.user_id);
      ...
      toast.error(`Sign-in Otp failed: ${response_data?.response?.data?.message}`);
      setErrorMessage(response_data?.response?.data?.message)
      setOtpError(true)
    }
    setOtpLoading(false)
  ...
  return (
    <section className="bg-slate-60 p-10 ">
