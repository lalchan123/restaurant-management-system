...
import { calculatedPrice, getAPIPostDataByRefId } from "@/helpers";
...
const ProductWishListCard = async ({ dish }) => {
  // const { images, name, price, review, restaurant_id, restaurant } = dish;

  // const discountedPrice = calculatedPrice(dish);
  console.log("19 dish", dish);

  const restaurantsData = await getAPIPostDataByRefId(40, dish?.Restuarant_Name, "0");
        ...
        <div className="flex flex-col sm:flex-row sm:items-center">
          ...
          <div className="w-2/3 md:w-auto">
            <p className="mb-2 text-xs font-medium text-primary">
              {restaurantsData?.data?.[0]?.First_Name} {restaurantsData?.data?.[0]?.Last_Name}
            </p>
