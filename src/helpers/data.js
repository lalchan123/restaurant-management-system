...
export const getFilteredProducts1Backup = (filter) => {
...
export const getFilteredProducts = (filter) => {
  const products = filter?.dishesData;
  // console.log("37 products", products);
  filter = filter ?? {};
  // console.log("39 filter", filter);
  return (products || []).filter((dish) => {
    return !(
      (filter.categories &&
        !!filter.categories.length &&
        !filter.categories.includes(dish.Product_Catagory)) ||
      (filter.minPrice && !(dish.Selling_Price >= filter.minPrice)) ||
      (filter.maxPrice && !(dish.Selling_Price <= filter.maxPrice))
      // (filter.search &&
      //   !dish.name.toLowerCase().includes(filter.search.toLowerCase())) ||
      // (filter.rating && !(dish.review.stars >= filter.rating))
    );
...
export const getAPIPostDataByRefId = async (table_id, table_ref_id, user_id) => {
  // You can fetch data from your server here
  await sleep(200);
  console.log("108 table_id table_ref_id user_id", table_id, table_ref_id, user_id)
  ...
  const data = await restAPIPost(apiUrl, payload);
